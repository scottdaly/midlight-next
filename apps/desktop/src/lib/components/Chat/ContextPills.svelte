<script lang="ts">
  ...
  function getIcon(type: ContextItem['type']) {
    switch (type) {
      case 'file':
        return 'file';
      case 'selection':
...
{#if $ai.contextItems.length > 0}
  <div class="flex flex-wrap gap-1.5 px-3 py-2 border-b border-border">
    {#each $ai.contextItems as item, i}
      <div class="flex items-center gap-1 bg-accent rounded-full pl-2 pr-1 py-0.5 text-xs">
        {#if item.type === 'file'}
        ...
        {/if}
        <span class="max-w-32 truncate">{getDisplayName(item)}</span>
        <button
